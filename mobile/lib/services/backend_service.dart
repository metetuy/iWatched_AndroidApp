...
class BackendService {
  ...
  Future<List<(int, int)>> fetchBatchRecommendations(
      String userId, bool isInitialFetch,
      {int count = 5}) async {
    debugPrint("═══════════════════════════════════════════");
    ...
    try {
      ...
      debugPrint(
          "   Is the backend running? Try: uvicorn main:app --host 0.0.0.0 --port 5000");
  ...
  Future<int?> lookupMovieIndex(int tmdbId) async {
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/lookup/$tmdbId'))
      ...
      debugPrint("Lookup error for TMDB $tmdbId: $e");
    }
    return null;
  }
}
